...
* publishing
Map over all subheadings of scraps and export each to a separate file.
#+begin_src emacs-lisp
  (defun dump-subtree-to-file ()
    (let* ((replacements '((" "   . "-")
           ...
           (path (expand-file-name (concat (downcase title) ".html") "scraps"))
           buf)
      (unless (file-exists-p path)
        (org-copy-subtree)
        (with-temp-buffer
          (yank) (setq buf (htmlize-buffer)))
        (set-buffer buf)
        (write-file path))))
